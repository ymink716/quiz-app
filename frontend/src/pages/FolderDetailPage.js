...
function FolderDetailPage() {
  ...
  useEffect(async () => {
    try {
      const folderRes = await axios.get(`/api/folders/${folderId}`, { headers: { Authorization: token }});

      if (folderRes.data.success) setFolder(folderRes.data.folder);
      else alert(folderRes.data.message);
      
      const unitsRes = await axios.get(`/api/units/folder/${folderId}`);

      if (unitsRes.data.units && unitsRes.data.units.length !== 0) setUnits([...unitsRes.data.units]);
    } catch (error) {
      console.error(error);
      alert('에러가 발생했습니다.');
    }
  ...
  const deleteFolderHandler = () => {
    ...
    axios.delete(`/api/folders/${folderId}`, { headers: {Authorization: token }})
    .then((response) => {
      if (response.data.success) navigate('/myFolders');
    ...
    <div style={{ width: '100%', marginLeft: '5%' }}>
      ...
      >
        ...
        >
          <Form.Item 
            label="제목" name="title"
            rules={[{ required: true, message: '제목(50글자 이내)', max: 50 }]}
          > 
            <Input /> 
          </Form.Item>
            <Form.Item 
